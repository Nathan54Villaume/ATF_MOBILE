...
fun DashboardATSScreen(navController: NavController, viewModel: SelectionViewModel) {
    val trefileuses = listOf(
        "T1" to "DB2020", "T2" to "DB2022", "T3" to "DB2024", "T4" to "DB2026",
        "T5" to "DB2028", "T6" to "DB2030", "T7" to "DB2033", "T8" to "DB2035"
    )
    val soudeuses = listOf(
        "S1" to "DB2040", "S2" to "DB2042", "S3" to "DB2045"
    )
    ...
    LaunchedEffect(Unit) {
        while (true) {
            trefData = ApiAutomateClient.fetchGroupedValues(trefileuses.associate { (label, db) ->
                db to listOf(
                    "$db.DBB0", "$db.DBD2", "$db.DBD6", "$db.DBD10", "$db.DBD14", "$db.DBD18"
                )
            })

            soudData = ApiAutomateClient.fetchGroupedValues(soudeuses.associate { (label, db) ->
                db to listOf(
